...
st.markdown("---")
st.subheader("Regel-Konklusion (Ziel)")
...
if st.button("Auswertung starten", type="primary"):
    ...
    if w_vdash_a:
        ...
        befund = "Die Aussage ist aus dem gegebenen Wissen ableitbar."
    else:
        if w_is_empty:
            status_text = "D2: Unbeweisbar bezüglich W (~(W ⊢ A))"
            befund = "Das Wissensarchiv W ist leer."
        else:
            status_text = "D5: Unentscheidbar bezüglich W"
            befund = "Die vorhandenen Fakten greifen nicht oder besitzen keinen Bezug."
